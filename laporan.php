...
if( empty( $_SESSION['id_user'] ) ){
	...
	die();
      ...
      if(isset($_REQUEST['submit'])){
		 ...
		 $sql = mysqli_query($koneksi, "SELECT * FROM ajc_transaksi WHERE tanggal BETWEEN '$tgl1' AND '$tgl2'");
	 ...
	 echo '
		...
		<div class="col-sm-6"><table class="table table-bordered">';
		 ...
		 $sql = mysqli_query($koneksi, "SELECT count(nama), sum(total) FROM ajc_transaksi WHERE tanggal BETWEEN '$tgl1' AND '$tgl2'");
...
<?php
	  ...
	  $sql = mysqli_query($koneksi, "SELECT count(nama), sum(total) FROM ajc_transaksi WHERE tanggal='$tanggal'");
